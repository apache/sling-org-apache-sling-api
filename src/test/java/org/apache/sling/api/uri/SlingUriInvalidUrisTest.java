...
public class SlingUriInvalidUrisTest {
    ...
    public static Collection<String[]> data() {
        return Arrays.asList(
                ...
                new String[] {
                    "http://example.com/path/with/spaces/A name with spaces.pdf", "http://example.com/test.pdf"
                },
                new String[] {
                    "http://user:pw@example.com/path/with/spaces/A name with spaces.sel1.pdf/suffix?par1=val1&par2=val2#frag",
    ...
    public void testInvalidUriToStringIsUnchanged() {
        try {
            ...
            assertEquals(
                    "Invalid URI " + invalidUri + "(e=" + e + ") is unchanged for SlingUriBuilder parse/toString",
                    invalidUri,
    ...
    public void testAdjustInvalidUriToValidUri() {
        ...
        SlingUri slingUriAdjusted =
                slingUri.adjust(b -> b.setSchemeSpecificPart(null).setResourcePath("/test"));
        assertEquals(
                "Using setSchemeSpecificPart(null) should reset the invalid URI to be adjustable",
                "/test",
    ...
    public void testAdjustInvalidUri() {
        ...
        assertEquals(
                "setResourcePath('/test') to invalid URI '" + invalidUri + "' should result in: "
                        + invalidUriAdjustedAfterSetPath,
                invalidUriAdjustedAfterSetPath,
                slingUriAdjusted.toString());
    }
}
